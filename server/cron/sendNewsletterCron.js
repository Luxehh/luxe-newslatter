...
const getTemplatesFromDatabase = async () => {
  try {
    const templates = await NewsletterTemplate.find({ isActive: true }).sort({
      orderNumber: 1,
    });
...
const shouldSendToday = (createdAt) => {
  ...
  const isRegisteredYesterday =
    createdDate.getTime() === yesterdayDate.getTime();
  if (isRegisteredYesterday) {
    console.log(
      `🎯 User registered yesterday - sending first newsletter today (day after registration)`
    );
    return true;
  }
...
const sendNewsletterMessage = async (subscriber, orderNumber, templateLink) => {
  try {
    const messageBody = `Hello ${subscriber.firstName}! \n\nHere's your #${orderNumber} Months Breavement Newsletter from Luxe Hospice
link ${templateLink}\n\nLuxe Hospice is here to support you, You are not alone`;
    ...
    console.log(
      `✅ Newsletter sent to ${subscriber.firstName} ${subscriber.lastName} (${subscriber.phoneNumber}) - Order ${orderNumber} - SID: ${message.sid}`
    );
    return true;
  } catch (error) {
    console.error(
      `❌ Failed to send newsletter to ${subscriber.phoneNumber}: ${error.message}`
    );
    return false;
  }
};
...
const sendMonthlyNewsletters = async () => {
  console.log(
    `📰 Running monthly newsletter job at ${new Date().toISOString()}`
  );
  ...
  try {
    ...
    if (templatesCount === 0) {
      console.log(
        "⚠️ No templates found in database, using fallback templates"
      );
    ...
    for (const subscriber of subscribers) {
      ...
      if (orderNumber === 0) {
        console.log(
          `⏭️ Subscriber ${subscriber.phoneNumber} registered today - will send first newsletter tomorrow`
        );
        skippedCount++;
        continue;
      }
      ...
      if (!shouldSendToday(subscriber.createdAt)) {
        console.log(
          `⏭️ Not the right day for ${subscriber.phoneNumber} - skipping`
        );
        skippedCount++;
        continue;
      }
    ...
    console.log(
      `✅ Newsletter job completed: ${sentCount} sent, ${skippedCount} skipped`
    );
  } catch (error) {
